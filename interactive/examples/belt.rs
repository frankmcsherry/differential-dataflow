...
fn build_line(n: i64) -> Topo {
    ...
    Topo {
        ...
        gate: (n - 1, n),
        cells: (0..n).collect(),
        rows: vec![("belt".into(), (0..n).collect())],
    }
}
...
fn main() {
    ...
    let fill_full = std::env::var("BELT_FILL")
        .map(|s| s == "full")
        .unwrap_or(false);
    ...
    timely::execute_directly(move |worker| {
        ...
        if fill_full {
            occ.extend(topo.cells.iter().copied());
        } else {
            occ.extend(topo.sources.iter().copied());
        }
        for &c in &occ {
        ...
        for tick in 0..ticks {
            if tick == close_at && gate_open {
                succ_in.update(edge(topo.gate.0, topo.gate.1), -1);
                gate_open = false;
            }
            if tick == open_at && !gate_open {
                succ_in.update(edge(topo.gate.0, topo.gate.1), 1);
                gate_open = true;
            }
            ...
            let targets: BTreeSet<i64> = moves
                .iter()
                .filter(|&&(_, t)| t != topo.drain)
                .map(|&(_, t)| t)
                .collect();
            let mut next: BTreeSet<i64> = occ.difference(&sources).copied().collect();
            next.extend(targets);
            next.extend(topo.sources.iter().copied()); // keep feeders fed
            for &c in next.difference(&occ) {
                occ_in.update(cell(c), 1);
            }
            for &c in occ.difference(&next) {
                occ_in.update(cell(c), -1);
            }
            occ = next;

            let time = (tick as u64) + 2;
            succ_in.advance_to(time);
            succ_in.flush();
            occ_in.advance_to(time);
            occ_in.flush();
            steps = 0;

...
fn first_int(v: &Value) -> i64 {
    match v {
        Value::Int(n) => *n,
        Value::Tuple(f) => first_int(&f[0]),
        other => panic!("no int in {:?}", other),
    }
}
...
fn rates_now(counts: &BTreeMap<(i64, i64), Diff>) -> BTreeMap<i64, i64> {
    ...
    for (&(stage, rate), &c) in counts {
        if c > 0 {
            out.insert(stage, rate);
        }
    }
    out
}
...
fn row_str(vals: &BTreeMap<i64, i64>, n: i64) -> String {
    (0..n)
        .map(|s| format!("{:>3}", vals.get(&s).copied().unwrap_or(0)))
        .collect::<Vec<_>>()
        .join(" ")
}
...
fn main() {
    let path = std::env::args()
        .nth(1)
        .unwrap_or_else(|| "examples/programs/rate_prefix.ddp".to_string());
    let n: i64 = std::env::var("FACTORY_N")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(8);
    let delay_ms: u64 = std::env::var("BELT_DELAY_MS")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(900);
    ...
    let script: Vec<(usize, i64, &str)> = vec![
        (
            5,
            20,
            "raise a NON-bottleneck (stage 5): nothing binds -> expect no work",
        ),
        (
            2,
            7,
            "raise THE bottleneck (stage 2 -> 7): throughput climbs",
        ),
        (
            2,
            9,
            "raise it again (stage 2 -> 9): line now balanced at 9",
        ),
        (
            6,
            2,
            "DROP a DOWNSTREAM stage (stage 6 -> 2): watch who recomputes",
        ),
    ...
    timely::execute_directly(move |worker| {
        ...
        let cap_row = |s: i64, c: i64| {
            (
                Value::Tuple(vec![Value::Int(s), Value::Int(c)]),
                Value::unit(),
            )
        };
        ...
        for s in 0..n {
            inputs[0].update(cap_row(s, caps[s as usize]), 1);
        }
        if n_inputs > 1 {
            for i in 0..n - 1 {
                inputs[1].update(edge(i, i + 1), 1);
            }
        }

        for h in inputs.iter_mut() {
            h.advance_to(1);
            h.flush();
        }
        let mut steps = 0u64;
        ...
        println!(
            "rate: {}    throughput = {}\n",
            row_str(&rates, n),
            rates.get(&(n - 1)).copied().unwrap_or(0)
        );
        ...
        for (ti, (stage, newcap, desc)) in script.iter().enumerate() {
            ...
            let mark: String = (0..n)
                .map(|s| if touched.contains(&s) { " ^^" } else { "  ." })
                .collect::<Vec<_>>()
                .join("");
            println!(
                "  DD  :{}   {} stages recomputed, {} record-changes ({} steps)",
                mark,
                touched.len(),
                seen.len(),
                steps

...
//! [`CorgiDistinctTactic`]) over the columnar chunks.
...
use crate::corgi::reduce::CorgiDistinctTactic;
...
impl Backend for CorgiBackend {
    ...
    fn reduce<'s>(a: Self::Arr<'s>, reducer: &Reducer) -> Self::Arr<'s> {
        reduce_with_tactic::<_, CTrace, _>(a, "CorgiReduce", CorgiDistinctTactic::new(reducer.clone()))
    }

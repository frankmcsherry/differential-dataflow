...
fn main() {
    ...
    timely::execute_from_args(std::env::args(), move |worker| {
        ...
        worker.dataflow::<u64, _, _>(|scope| {
            ...
            let data_pact = ColPact { hashfunc: |k: columnar::Ref<'_, Vec<u8>>| k.hashed() };
            let keys_pact = ColPact { hashfunc: |k: columnar::Ref<'_, Vec<u8>>| k.hashed() };

            let data = arrange_core::<_,_,ValBatcher<_,(),_,_>, ValBuilder<_,(),_,_>, ValSpine<_,(),_,_>>(&data, data_pact, "Data");
            let keys = arrange_core::<_,_,ValBatcher<_,(),_,_>, ValBuilder<_,(),_,_>, ValSpine<_,(),_,_>>(&keys, keys_pact, "Keys");
        ...
        let mut builder = ValColBuilder::<WordCount>::default();
...
mod test {
    ...
    use columnar::{Columnar, Borrow, Vecs, primitive::offsets::Strides, bytes::stash::Stash};
    use crate::ColumnarUpdate;

    pub type Lists<C> = Vecs<C, Strides>;
    pub type ColContainer<K,V,T,R> = Lists<(<K as Columnar>::Container, Lists<(<V as Columnar>::Container, Lists<(<T as Columnar>::Container, Lists<<R as Columnar>::Container>)>)>)>;

    struct Container<U: ColumnarUpdate> {
        stash: Stash<ColContainer<U::Key, U::Val, U::Time, U::Diff>, timely::bytes::arc::Bytes>,
    }

    impl<U: ColumnarUpdate> Container<U> {
        /// Borrows `self` as its constituent borrowed lists.
        fn borrow_parts(&self) -> (
            <Lists<<U::Key as Columnar>::Container> as Borrow>::Borrowed<'_>,
            <Lists<<U::Val as Columnar>::Container> as Borrow>::Borrowed<'_>,
            <Lists<<U::Time as Columnar>::Container> as Borrow>::Borrowed<'_>,
            <Lists<<U::Diff as Columnar>::Container> as Borrow>::Borrowed<'_>,
        ) {
            let rest = self.stash.borrow();
            let (keys, rest) = (Vecs { bounds: rest.bounds, values: rest.values.0 }, rest.values.1);
            let (vals, rest) = (Vecs { bounds: rest.bounds, values: rest.values.0 }, rest.values.1);
            let (times, diffs) = (Vecs { bounds: rest.bounds, values: rest.values.0 }, rest.values.1);

            (keys, vals, times, diffs)
        }
        /// Assembles an owned self from constituent owned lists.
        fn from_parts(lists: (Lists<<U::Key as Columnar>::Container>, Lists<<U::Val as Columnar>::Container>, Lists<<U::Time as Columnar>::Container>, Lists<<U::Diff as Columnar>::Container>)) -> Self {
            let (keys, vals, times, diffs) = lists;

            Self { stash: Stash::Typed(Vecs {
                bounds: keys.bounds,
                values: (keys.values,
                    Vecs {
                        bounds: vals.bounds,
...
pub use column_builder::val::ValBuilder as ValColBuilder;
mod column_builder {
...
mod distributor {
    ...
    pub mod key {
        ...
        pub struct ColDistributor<U: Update, H> {
            marker: std::marker::PhantomData<U>,
            hashfunc: H,
        }

        pub struct ColPact<H> { pub hashfunc: H }

        impl<U: Update, H: for<'a> FnMut(columnar::Ref<'a, U::Key>)->u64> Distributor<ValStorage<U>> for ColDistributor<U, H> {
            fn partition<T: Clone, P: timely::communication::Push<Message<T, ValStorage<U>>>>(&mut self, container: &mut ValStorage<U>, time: &T, pushers: &mut [P]) {
                ...
                let in_vals = container.vals.borrow();
                ...
                let mut out_vals = vec![Vecs::<ContainerOf::<U::Val>>::default(); pushers.len()];
                ...
                for index in 0 .. in_keys.len() {
                    ...
                    out_vals[idx].extend_from_self(in_vals, index..index+1);
                    out_upds[idx].extend_from_self(in_upds, container.vals_bounds(index .. index+1));
                }

                for (((pusher, keys), vals), upds) in pushers.iter_mut().zip(out_keys).zip(out_vals).zip(out_upds) {
                    let mut container = ValStorage { keys: keys.into(), vals: vals.into(), upds: upds.into() };
        ...
        impl<T, U, H> ParallelizationContract<T, ValStorage<U>> for ColPact<H>
        where
            T: Timestamp,
            U: Update,
            H: for<'a> FnMut(columnar::Ref<'a, U::Key>)->u64 + 'static,
        {
            ...
            fn connect<A: AsWorker>(self, allocator: &mut A, identifier: usize, address: Rc<[usize]>, logging: Option<TimelyLogger>) -> (Self::Pusher, Self::Puller) {
                ...
                let distributor = ColDistributor {
...
pub use arrangement::{ValBatcher, ValBuilder, ValSpine, KeyBuilder, KeySpine};
pub mod arrangement {
    ...
    pub mod batch_container {
        ...
        impl<C: Columnar + Ord + Clone> BatchContainer for Coltainer<C> where for<'a> columnar::Ref<'a, C> : Ord {
            ...
            fn merge_capacity(cont1: &Self, cont2: &Self) -> Self {
                Self { container: <C as Columnar>::Container::with_capacity_for([cont1.container.borrow(), cont2.container.borrow()].into_iter()) }
            }
    ...
    use crate::{ColumnarUpdate, ValStorage};
    ...
    pub mod builder {
        ...
        pub mod key {
            ...
            impl<U: Update<Val=()>> differential_dataflow::trace::Builder for KeyMirror<U> {
                type Time = U::Time;
                type Input = ValStorage<(<U as Update>::Key, <U as Update>::Val, <U as Update>::Time, <U as Update>::Diff)>;

...
mod identity {
    ...
    pub struct IdentityReduce<T, L> {
        pub logic: L,
        /// The output tiles of the session in progress: emitted records per description.
        tiles: Vec<(Description<T>, Vec<((u64, u64), T, isize)>)>,
        ...
        L: FnMut(&[(u64, isize)]) -> Vec<(u64, isize)>,
    {
        ...
        fn next_window(&mut self, instance: &ReduceInstance<'_, Batch<T>, Batch<T>>, changed: &[u64], cursor: &mut usize) -> Option<ReduceWindow<T, isize, isize>> {
            ...
            // Deliberately tight windows (2 keys), so the fuzz exercises the multi-window path.
        ...
        // The bulk crossing: per bracket, desired = logic(input accumulation), and the
        // correction is desired − current (the ids are the values; no resolution needed).
        // Asserts the bracket protocol from the backend's seat.
        fn reduce_corrections(
            &mut self,
            keys: &[u64],
            in_ends: &[usize],
            ...
            assert_eq!(keys.len(), in_ends.len(), "one input bracket end per key");
            assert_eq!(keys.len(), out_ends.len(), "one output bracket end per key");
            assert!(in_ends.windows(2).all(|w| w[0] <= w[1]), "input brackets increasing");
            assert_eq!(in_ends.last().copied().unwrap_or(0), input.len(), "input brackets cover the input");
            assert_eq!(out_ends.last().copied().unwrap_or(0), output.len(), "output brackets cover the output");
            let mut corr = Vec::new();
            let mut corr_ends = Vec::with_capacity(keys.len());
            let (mut i0, mut o0) = (0, 0);
            for i in 0..keys.len() {
                let in_bracket = &input[i0..in_ends[i]];
                let out_bracket = &output[o0..out_ends[i]];
                i0 = in_ends[i];
                o0 = out_ends[i];
                let mut delta = if in_bracket.is_empty() { Vec::new() } else { (self.logic)(in_bracket) };
                delta.extend(out_bracket.iter().map(|&(v, d)| (v, -d)));
        ...
        fn finish(&mut self) -> Vec<Batch<T>> {
            ...
            for (description, mut rows) in self.tiles.drain(..) {
                ...
                batches.push(Rc::new(ChunkBatch::new(chunks, description)));
            }
            batches
        }
    }

...
use differential_dataflow::trace::chunk::{pack, Chunk};
...
impl<T: Clone + Ord, R: Clone> OutBuf<T, R> {
    ...
    fn flush(&mut self, force: bool, out: &mut VecDeque<IdentityChunk<T, R>>) {
        if self.len() >= TARGET || (force && self.len() > 0) {
            let full = std::mem::replace(self, OutBuf::new());
            out.push_back(IdentityChunk(Rc::new(Inner {
                keys: full.keys,
                vals: full.vals,
                times: full.times,
                diffs: full.diffs,
    ...
    R: Semigroup + Clone + 'static,
{
    ...
    fn advance(
        ...
        if !done && (keys[0], vals[0]) == (keys[n - 1], vals[n - 1]) {
            input.push_front(IdentityChunk(Rc::new(Inner { keys, vals, times, diffs })));
            return;
        }
        ...
        if end < n {
            input.push_front(IdentityChunk(Rc::new(Inner {
    ...
    fn settle(input: &mut VecDeque<Self>, done: bool, out: &mut VecDeque<Self>) {
        pack(
            ...
            |acc, next| {
                ...
                t.append(&mut t2);
                d.append(&mut d2);
                *acc = IdentityChunk(Rc::new(Inner { keys: k, vals: v, times: t, diffs: d }));
            },
            |chunk, m| {
                let n = chunk.len();
...
mod test {
    ...
    use super::{Chunk, IdentityChunk};
    use differential_dataflow::trace::chunk::{is_graded, ChunkBatch, ChunkBatchMerger};
    ...
    fn read_batch(b: &ChunkBatch<IdentityChunk<u64, i64>>) -> BTreeMap<((u64, u64), u64), i64> {
        let mut m = BTreeMap::new();
        for ch in &b.chunks {
            for i in 0..IdentityChunk::len(ch) {
    ...
    fn batch(rows: &[((u64, u64), u64, i64)], sz: usize) -> ChunkBatch<IdentityChunk<u64, i64>> {
        let mut m: BTreeMap<((u64, u64), u64), i64> = BTreeMap::new();
        for &(kv, t, d) in rows { *m.entry((kv, t)).or_insert(0) += d; }
        m.retain(|_, d| *d != 0);
    ...
    fn from_unsorted_consolidates_with_provenance() {
        ...
        let (chunk, reps) = IdentityChunk::from_unsorted(keys, vals, times, diffs);
    ...
    fn settle_grades_and_preserves() {
        ...
        IdentityChunk::settle(&mut input, true, &mut out);
